...
void Xicro::_SendUint8(uint8_t *data,uint8_t len){
    if(len>1){
        ...
        HAL_UART_Transmit(_serial, data, len, 1000);
    }else{
        uint8_t buff[2]={8,0};
...
void Xicro::_SendInt8(int8_t *data,uint8_t len){
    if(len>1){
        ...
        HAL_UART_Transmit(_serial, (uint8_t*)data, len, 1000);
    }else{
        uint8_t buff[2]={18,0};
        buff[1]=(int8_t)*data & 0xFF;
        _crc=_getcrc(_crc,buff,2);
        HAL_UART_Transmit(_serial, buff, 2, 1000);
    }
}
void Xicro::_SendInt16(int16_t *data,uint8_t len){
    if(len>1){
        ...
        uint8_t sp[sizee];
        for (int i=0;i<len;i++){
            sp[(i*2)+1] = (int16_t)data[i] >> 0 & 0xFF;
            sp[(i*2)]   = (int16_t)data[i] >> 8 & 0xFF;
...
void Xicro::_SendInt64(int64_t *data,uint8_t len){
    if(len>1){
        ...
        uint8_t sp[sizee];
        for (int i=0;i<len;i++){
            sp[(i*8)+7] = (int64_t)data[i] >> 0  & 0xFF;
            sp[(i*8)+6] = (int64_t)data[i] >> 8  & 0xFF;
            sp[(i*8)+5] = (int64_t)data[i] >> 16 & 0xFF;
            sp[(i*8)+4] = (int64_t)data[i] >> 24 & 0xFF;  
            sp[(i*8)+3] = (int64_t)data[i] >> 32 & 0xFF;
            sp[(i*8)+2] = (int64_t)data[i] >> 40 & 0xFF;
            sp[(i*8)+1] = (int64_t)data[i] >> 48 & 0xFF;
            sp[(i*8)]   = (int64_t)data[i] >> 56 & 0xFF;     
        }
        _crc=_getcrc(_crc,sp,sizee);
        HAL_UART_Transmit(_serial, sp, sizee, 1000);
        ...
        uint8_t buff[9]={64,0,0,0,0,0,0,0,0};
        buff[8] = (int64_t)*data >> 0  & 0xFF;
        buff[7] = (int64_t)*data >> 8  & 0xFF;
        buff[6] = (int64_t)*data >> 16 & 0xFF;
        buff[5] = (int64_t)*data >> 24 & 0xFF;
        buff[4] = (int64_t)*data >> 32 & 0xFF;
        buff[3] = (int64_t)*data >> 40 & 0xFF;
        buff[2] = (int64_t)*data >> 48 & 0xFF;
        buff[1] = (int64_t)*data >> 56 & 0xFF;
        _crc=_getcrc(_crc,buff,9);
        HAL_UART_Transmit(_serial, buff, 9, 1000);
...
void Xicro::_SendBool(bool *data,uint8_t len,bool flagNext){ //flagnext=1=continue,flagnext=0=stop
    if(len>1){
        ...
        HAL_UART_Transmit(_serial, buff, sizee, 1000);
